...
**Source:** [`@mrquake/quakecode-tui`](https://github.com/mrquakex5/quake-code/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@mrquake/quakecode-tui";
...
```typescript
import { Container, type Focusable, Input } from "@mrquake/quakecode-tui";
...
Import from `@mrquake/quakecode-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@mrquake/quakecode-tui";
```
...
```typescript
import { matchesKey, Key } from "@mrquake/quakecode-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@mrquake/quakecode-tui";
...
```typescript
import { getMarkdownTheme } from "@mrquake/quakecode-cli";
import { Markdown } from "@mrquake/quakecode-tui";
...
Set `QUAKE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
QUAKE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@mrquake/quakecode-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@mrquake/quakecode-cli";
import { DynamicBorder } from "@mrquake/quakecode-cli";
import { Container, type SelectItem, SelectList, Text } from "@mrquake/quakecode-tui";
...
```typescript
import { BorderedLoader } from "@mrquake/quakecode-cli";

quake.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@mrquake/quakecode-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@mrquake/quakecode-cli";
import { Container, type SettingItem, SettingsList, Text } from "@mrquake/quakecode-tui";

quake.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
import { matchesKey, truncateToWidth } from "@mrquake/quakecode-tui";
...
Extensions with clickable regions should implement `MouseTarget`:

```typescript
import { type HitRegion, type MouseCollectContext, type MouseTarget } from "@mrquake/quakecode-tui";

class MyButton implements Component, MouseTarget {
...
Rebuild a `SpatialIndex` in a TUI `addAfterRenderListener` callback — never call `render()` again on click. Parse stdin with `parseSgrMouse()`. See [keyboard-shortcuts.md](keyboard-shortcuts.md) and [qa.md](qa.md).
