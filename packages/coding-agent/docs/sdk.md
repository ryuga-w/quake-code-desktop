...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@mrquake/quakecode-cli";
...
```bash
npm install @mrquake/quakecode-cli
```
...
```typescript
import { createAgentSession } from "@mrquake/quakecode-cli";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `quake.sendMessage()`.
...
The `Agent` class (from `@mrquake/quakecode-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.quake-code/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.quake-code/extensions/`)
- Project skills:
  - `.quake-code/skills/`
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.quake-code/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@mrquake/quakecode-cli";
...
// Default: uses ~/.quake-code/agent/auth.json and ~/.quake-code/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@mrquake/quakecode-cli";
...
Default agent guidance follows Codex-style text inspection: it uses `bashTool` with commands such as `cat`, `sed`, `nl`, `head`, `tail`, `rg`, `wc`, and `git show`. `readTool` remains available for image attachments, exact `offset`/`limit` paging, and SDK compatibility.
...
import { createAgentSession, defineTool } from "@mrquake/quakecode-cli";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `quake.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `quake.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.quake-code/agent/extensions/`, `.quake-code/extensions/`, and settings.json extension sources.
...
**Event Bus:** Extensions can communicate via `quake.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@mrquake/quakecode-cli";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@mrquake/quakecode-cli";
...
1. Global: `~/.quake-code/agent/settings.json`
2. Project: `<cwd>/.quake-code/settings.json`
