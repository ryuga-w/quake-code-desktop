...
Use `/logout` to clear credentials. Tokens are stored in `~/.quake-code/agent/auth.json` and auto-refresh when expired.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/mrquakex5/quake-code/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/mrquakex5/quake-code/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.quake-code/agent/auth.json`:
...
OpenRouter is the easiest way to use Qwen and other non-default model families in Quake.
...
Then run any OpenRouter model directly:
...
Quake can also fall back to a custom model ID when an OpenRouter model exists but is not yet bundled in the local model registry.
