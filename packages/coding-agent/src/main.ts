...
Options:
  -l, --local    Install project-locally (.quake-code/settings.json)
...
Options:
  -l, --local    Remove from project settings (.quake-code/settings.json)
...
async function handleWebCommand(args: string[]): Promise<boolean> {
	if (args[0] !== "web") return false;
	const passthrough = args.slice(1);
	const child = spawn(
		process.platform === "win32" ? "npm.cmd" : "npm",
		["--workspace", "@mrquake/quakecode-web", "run", "dev", "--", ...passthrough],
		{
			stdio: "inherit",
			cwd: resolve(process.cwd()),
			env: { ...process.env, QUAKE_WEB_CWD: process.env.QUAKE_WEB_CWD ?? process.cwd() },
		},
	);
...
export async function main(args: string[]) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.QUAKE_CODE_OFFLINE);
	if (offlineMode) {
		process.env.QUAKE_CODE_OFFLINE = "1";
		process.env.QUAKE_CODE_SKIP_VERSION_CHECK = "1";
	}
	...
	// MCP Memory Server mode: start memory server and listen on stdin
	if (args[0] === "mcp-memory-server" || args[0] === "mcp-memory") {
		const { startMemoryMcpServer } = await import("./core/memory-mcp-server.js");
		console.error("Quake Code Memory MCP server started. Waiting for JSON-RPC on stdin...");
	...
	// Early exit for simple commands that don't need resource loading
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.QUAKE_CODE_STARTUP_BENCHMARK);
	if (startupBenchmark && !isInteractive) {
		console.error(chalk.red("Error: QUAKE_CODE_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
