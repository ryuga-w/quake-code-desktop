...
4. Get the commit SHA: `cd /tmp/quake-github-repos/owner/repo && git rev-parse HEAD`
...
```bash
cd /tmp/quake-github-repos/owner/repo
...
```bash
cd /tmp/quake-github-repos/owner/repo && git rev-parse HEAD
```
...
| gh CLI rate limited | Use the already-cloned repo in /tmp/quake-github-repos/ for git operations |
