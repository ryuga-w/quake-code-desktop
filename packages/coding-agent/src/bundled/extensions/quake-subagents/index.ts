/**
 * quake-subagents — A Quake Code extension providing Claude Code-style autonomous sub-agents.
 *
 * Tools:
...
import type { ExtensionAPI, ExtensionCommandContext, ExtensionContext } from "@mrquake/quakecode-cli";
import { Text } from "@mrquake/quakecode-tui";
import { Type } from "@sinclair/typebox";
import { registerAgentJobTools } from "./agent-jobs.js";
...
import { agentJobsEnabled, formatCodexV1Notification, legacySubagentToolsEnabled } from "./codex-parity.js";
import { registerCodexMultiAgentTools } from "./codex-tools.js";
...
import { loadPersistedAgentRecords, SUBAGENT_RECORD_ENTRY, serializeAgentRecord } from "./persistence.js";
import { getSubagentRuntimeScope } from "./runtime-scope.js";
...
import { registerSubagentWebControl } from "./web-control.js";
...
/** Image attachments are optional in web/desktop tool contexts. */
export function getCurrentMessageImages(messages: ExtensionContext["messages"] | undefined): ImageContent[] {
	if (!Array.isArray(messages)) return [];
	const lastMessage = messages.at(-1);
	if (lastMessage?.role !== "user" || !Array.isArray(lastMessage.content)) return [];
	return lastMessage.content.filter((part): part is ImageContent => part.type === "image");
}
...
function formatTaskNotification(record: AgentRecord, resultMaxLen: number): string {
	...
	return [
		...
		`<summary>Agent ${escapeXml(record.name)} ("${escapeXml(record.description)}") ${record.status}</summary>`,
...
function buildDetails(
	base: Pick<AgentDetails, "name" | "displayName" | "description" | "subagentType" | "modelName" | "tags">,
	record: {
		toolUses: number;
		startedAt: number;
		completedAt?: number;
...
export default function (quake: ExtensionAPI) {
	const runtimeScope = getSubagentRuntimeScope();
	const ownsManager = runtimeScope == null;
	const enableLegacyTools = legacySubagentToolsEnabled();

	// ---- Register custom notification renderer ----
	quake.registerMessageRenderer<NotificationDetails>("subagent-notification", (message, { expanded }, theme) => {
		...
		function renderOne(d: NotificationDetails): string {
			...
			// Line 1: caller-assigned name + task description + status
			let line = `${icon} ${theme.bold(d.name)} ${theme.fg("dim", `· ${d.description} · ${statusText}`)}`;
	...
	/** Reload agents from .quake-code/agents/*.md and merge with defaults (called on init and each Agent invocation). */
	const reloadCustomAgents = (cwd = process.cwd()) => {
	...
	// before they reach quake.sendMessage (fire-and-forget).
	...
	function emitIndividualNudge(record: AgentRecord) {
		...
		quake.sendMessage<NotificationDetails>(
			{
	...
	const manager =
		...
		new AgentManager(
			(record) => {
				...
				if (isError) {
					quake.events.emit("subagents:failed", eventData);
				} else {
					quake.events.emit("subagents:completed", eventData);
				}

				// Nested completions are delivered directly to their parent session by the
				// shared manager. Do not also trigger an unrelated root follow-up turn.
				if (record.parentId) {
					agentActivity.delete(record.id);
					widget.markFinished(record.id);
					widget.update();
				...
				if (record.protocol === "codex-v1") {
					if (!record.completionNotified) {
						...
						quake.sendMessage(
							{
								customType: "subagent-notification",
								content: formatCodexV1Notification(record),
								display: false,
								details: buildNotificationDetails(record, 500, agentActivity.get(record.id)),
							},
							{ deliverAs: "steer", triggerTurn: false },
						);
						quake.appendEntry(SUBAGENT_RECORD_ENTRY, serializeAgentRecord(record));
					}
					agentActivity.delete(record.id);
					widget.markFinished(record.id);
					widget.update();
			...
			(record) => {
				...
				quake.events.emit("subagents:started", {
					id: record.id,
					name: record.name,
					type: record.type,
					description: record.description,
			...
			(record) => {
				if (record.protocol?.startsWith("codex-")) {
					quake.appendEntry(SUBAGENT_RECORD_ENTRY, serializeAgentRecord(record));
				}
			},
		);
	...
	if (ownsManager) {
		manager.setRootMessenger((message) => {
			quake.sendMessage(
				{ customType: "subagent-peer-message", content: message, display: false },
				{ deliverAs: "steer", triggerTurn: false },
			);
		});
	}

	// Every isolated child session registers the same Codex lifecycle surface, but
	// all of them share the root manager carried by runtimeScope.
	registerCodexMultiAgentTools(quake, { manager, runtimeScope });
	if (agentJobsEnabled()) registerAgentJobTools(quake, { manager, runtimeScope });

	// Expose only the root manager via Symbol.for() for cross-package compatibility.
	const MANAGER_KEY = Symbol.for("quake-subagents:manager");
	if (ownsManager) {
	...
	// --- Cross-extension RPC via quake.events ---
	let currentCtx: ExtensionContext | undefined;
	let unregisterSubagentWebControl: (() => void) | undefined;
	...
	quake.on("session_start", async (_event, ctx) => {
		...
		if (ownsManager) {
			unregisterSubagentWebControl?.();
			unregisterSubagentWebControl = registerSubagentWebControl({
				sessionId: ctx.sessionManager.getSessionId(),
				quake,
				manager,
				getContext: () => currentCtx,
			});
			manager.restorePersisted(loadPersistedAgentRecords(ctx.sessionManager.getEntries()), quake, ctx);
			manager.clearCompleted();
		}
	});

	quake.on("session_switch", () => {
		if (ownsManager) manager.clearCompleted();
	});

	const rpcHandlers = ownsManager
		? registerRpcHandlers({
				events: quake.events,
				quake,
				getCtx: () => currentCtx,
	...
	const { unsubPing: unsubPingRpc, unsubSpawn: unsubSpawnRpc, unsubStop: unsubStopRpc } = rpcHandlers;

	if (ownsManager) quake.events.emit("subagents:ready", {});
	...
	quake.on("session_shutdown", async () => {
		...
		if (ownsManager) {
			unregisterSubagentWebControl?.();
			unregisterSubagentWebControl = undefined;
			delete (globalThis as any)[MANAGER_KEY];
			manager.setRootMessenger(undefined);
			manager.abortAll();
			manager.dispose();
		}
	});
	...
	quake.on("tool_execution_start", async (_event, ctx) => {
	...
	const buildTypeListText = () => {
		...
		return [
			...
			"",
			"Custom agents can be defined in .quake-code/agents/<name>.md (project) or ~/.quake-code/agent/agents/<name>.md (global) — they are picked up automatically. Project-level agents override global ones. Creating a .md file with the same name as a default agent overrides it.",
		].join("\n");
	};
	...
	if (enableLegacyTools) {
		quake.registerTool<any, AgentDetails>({
			name: "Agent",
			label: "Agent",
...
Guidelines:
- Give every new agent a short, meaningful, unique name that reflects its responsibility. You choose the name; Quake never generates one.
...
- Parallel agents default to isolation: "worktree" (Codex-style isolated git copy). Use isolation: "none" only when sharing the main tree is intentional.`,
			parameters: Type.Object({
				name: Type.Optional(
					Type.String({
						description:
							"Required when launching a new agent: a short, meaningful, unique instance name chosen by you (for example Atlas, Nova, or AuthScout). Omit only when resuming an existing agent; Quake never generates names.",
						minLength: 1,
						maxLength: 48,
					}),
				),
				...
				subagent_type: Type.String({
					description: `The type of specialized agent to use. Available types: ${getAvailableTypes().join(", ")}. Custom agents from .quake-code/agents/*.md (project) or ~/.quake-code/agent/agents/*.md (global) are also available.`,
				}),
				model: Type.Optional(
					Type.String({
						description:
				...
				isolation: Type.Optional(
					Type.String({
						description:
							'Default is "worktree" (isolated git copy for safe parallel work). Set "none" to use the main working tree. On completion, worktree changes are saved to a branch.',
					}),
				),
			}),
			...
			renderCall(args, theme) {
				...
				const name = args.name?.trim() || "Unnamed";
				const desc = args.description ?? "";
				return new Text(
					`▸ ${theme.fg("toolTitle", theme.bold(name))} ${theme.fg("dim", `(${displayName})`)}${desc ? `  ${theme.fg("muted", desc)}` : ""}`,
					0,
					0,
				);
			},
			...
			execute: async (toolCallId, params, signal, onUpdate, ctx) => {
				...
				// Reload custom agents from the active workspace, not the server process cwd.
				reloadCustomAgents(ctx.cwd);

				const requestedName = params.name?.trim();
				if (!requestedName && !params.resume) {
					return textResult(
						"Subagent name is required. Choose a meaningful unique name; Quake will not generate one.",
					);
				}
				...
				const detailBase = {
					name: requestedName ?? (params.resume ? manager.getRecord(params.resume)?.name : undefined) ?? "Agent",
					displayName,
				...
				// Extract images from current message content if available.
				// This must be resolved before either foreground or background execution;
				// background agents pass the same image attachments into manager.spawn().
				const currentImages = getCurrentMessageImages(ctx.messages);
				...
				if (params.resume) {
					...
					return textResult(
						...
						buildDetails({ ...detailBase, name: existing.name }, record),
					);
				}
				...
				if (runInBackground) {
					...
					id = manager.spawn(quake, ctx, subagentType, params.prompt, {
						name: requestedName,
						description: params.description,
						parentId: runtimeScope?.currentAgentId,
						model,
					...
					// Emit created event
					quake.events.emit("subagents:created", {
						id,
						name: requestedName,
						type: subagentType,
						description: params.description,
					...
					if (signal?.aborted) manager.abort(id);
					const isQueued = record?.status === "queued";
					return textResult(
						`Agent ${requestedName} ${isQueued ? "queued" : "started"} in background.\n` +
							`Agent ID: ${id}\n` +
							`Type: ${displayName}\n` +
							`Description: ${params.description}\n` +
				...
				const record = await manager.spawnAndWait(quake, ctx, subagentType, params.prompt, {
					name: requestedName,
					description: params.description,
					parentId: runtimeScope?.currentAgentId,
					model,
			...
			execute: async (_toolCallId, params, _signal, _onUpdate, _ctx) => {
				...
				if (params.wait && (record.status === "running" || record.status === "queued")) {
					...
					while (record.status === "queued") {
						if (_signal?.aborted) return textResult(`Waiting for agent "${params.agent_id}" was cancelled.`);
		...
		quake.registerTool({
			...
			execute: async (_toolCallId, params, _signal, _onUpdate, _ctx) => {
				...
				if (!record.session) {
					// Session not ready yet (including queued agents) — deliver once initialized
					if (!record.pendingSteers) record.pendingSteers = [];
					record.pendingSteers.push(params.message);
					quake.events.emit("subagents:steered", { id: record.id, message: params.message });
					return textResult(
	...
	const projectAgentsDir = () => join(process.cwd(), ".quake-code", "agents");
	const personalAgentsDir = () => join(homedir(), ".quake-code", "agent", "agents");
	...
	async function ejectAgent(ctx: ExtensionCommandContext, name: string, cfg: AgentConfig) {
		const location = await ctx.ui.select("Choose location", [
			"Project (.quake-code/agents/)",
			"Personal (~/.quake-code/agent/agents/)",
		]);
		if (!location) return;
	...
	async function showGenerateWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		const generatePrompt = `Create a custom Quake sub-agent definition file based on this description: "${description}"
...
Write the file using the write tool. Only write the file, nothing else.`;
		...
		const record = await manager.spawnAndWait(quake, ctx, "general-purpose", generatePrompt, {
			name: `AgentDefinitionWriter-${name}`,
