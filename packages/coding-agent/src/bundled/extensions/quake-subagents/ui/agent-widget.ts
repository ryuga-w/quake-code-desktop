...
export interface AgentDetails {
	/** Caller-assigned instance name (for example, Atlas). */
	name: string;
	displayName: string;
	description: string;
	subagentType: string;
...
export class AgentWidget {
	...
	private renderFinishedLine(
		...
		const roleTag = ` ${theme.fg("dim", `(${name}${modeLabel ? ` · ${modeLabel}` : ""})`)}`;
		return `${icon} ${theme.bold(a.name)}${roleTag}  ${theme.fg("dim", a.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", parts.join(" · "))}${statusText}`;
	}
	...
	private renderWidget(tui: any, theme: Theme): string[] {
		...
		for (const a of running) {
			...
			runningLines.push([
				truncate(
					theme.fg("dim", "├─") +
						` ${theme.fg("accent", frame)} ${theme.bold(a.name)}${roleTag}  ${theme.fg("muted", a.description)} ${theme.fg("dim", "·")} ${theme.fg("dim", statsText)}`,
				),
