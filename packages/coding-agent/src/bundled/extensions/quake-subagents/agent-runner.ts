...
import type { ExtensionContext } from "@mrquake/quakecode-cli";
import {
	type AgentSession,
...
import { buildParentContext, extractText, seedForkedSession } from "./context.js";
import { detectEnv } from "./env.js";
import { buildMemoryBlock, buildReadOnlyMemoryBlock } from "./memory.js";
import { buildAgentPrompt, type PromptExtras } from "./prompts.js";
import { runWithSubagentRuntimeScope, type SubagentRuntimeScope } from "./runtime-scope.js";
...
export interface RunOptions {
	/** ExtensionAPI instance — used for quake.exec() instead of execSync. */
	quake: ExtensionAPI;
	model?: Model<any>;
	maxTurns?: number;
	signal?: AbortSignal;
	...
	/** Number of recent parent turns to inherit, or all available history. */
	forkTurns?: "all" | number;
	/** Seed inherited history as real session messages instead of prompt text. */
	forkAsSession?: boolean;
	/** Persist the child rollout so closed agents can be resumed later. */
	persistSession?: boolean;
	/** Reopen an existing persisted child rollout. */
	resumeSessionFile?: string;
	thinkingLevel?: ThinkingLevel;
	/** Shared control-plane identity used by recursively spawned agents. */
...
export async function runAgent(
	...
	type: SubagentType,
	prompt: string,
	options: RunOptions & { images?: ImageContent[] },
): Promise<RunResult> {
	if (options.runtimeScope) {
		return runWithSubagentRuntimeScope(options.runtimeScope, () => runAgentInternal(ctx, type, prompt, options));
	}
	return runAgentInternal(ctx, type, prompt, options);
}
...
async function runAgentInternal(
	...
	const env = await detectEnv(options.quake, effectiveCwd);
	...
	// Persisted sessions restore their saved model and thinking level unless the
	// caller explicitly overrides them.
	const model =
		options.model ??
		(options.resumeSessionFile ? undefined : resolveDefaultModel(ctx.model, ctx.modelRegistry, agentConfig?.model));
	const thinkingLevel = options.thinkingLevel ?? (options.resumeSessionFile ? undefined : agentConfig?.thinking);

	let sessionManager: SessionManager;
	if (options.resumeSessionFile) {
		sessionManager = SessionManager.open(options.resumeSessionFile);
	} else if (options.persistSession) {
		const sessionDir = join(homedir(), ".quake-code", "agent", "subagents", ctx.sessionManager.getSessionId());
		sessionManager = SessionManager.create(effectiveCwd, sessionDir);
	} else {
		sessionManager = SessionManager.inMemory(effectiveCwd);
	}
	if (!options.resumeSessionFile && options.inheritContext && options.forkAsSession) {
		seedForkedSession(ctx, sessionManager, options.forkTurns ?? "all");
	}
	...
	// Keep Codex-style lifecycle tools available for recursive delegation. Only
	// legacy Quake aliases are hidden to avoid competing tool surfaces in children.
	// Apply extension allowlists and disallowedTools after that compatibility filter.
	if (extensions !== false) {
	...
	if (options.inheritContext && !options.forkAsSession) {
		const parentContext = buildParentContext(ctx, options.forkTurns ?? "all");
		if (parentContext) {
	...
	try {
		await session.prompt(effectivePrompt, options.images?.length ? { images: options.images } : undefined);
	} finally {
		unsubTurns();
...
export async function resumeAgent(
	...
	try {
		await session.prompt(prompt, options.images?.length ? { images: options.images } : undefined);
	} finally {
		collector.unsubscribe();
		unsubToolUse();
		cleanupAbort();
