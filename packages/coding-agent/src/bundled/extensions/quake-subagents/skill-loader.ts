 ...
 *
 * When skills is a string[], reads each named skill from .quake-code/skills/ or ~/.quake-code/skills/
...
export function preloadSkills(skillNames: string[], cwd: string): PreloadedSkill[] {
	...
	for (const name of skillNames) {
		...
		if (content !== undefined) {
			...
			results.push({ name, content: `(Skill "${name}" not found in .quake-code/skills/ or ~/.quake-code/skills/)` });
		}
	}
...
function findAndReadSkill(name: string, cwd: string): string | undefined {
	const projectDir = join(cwd, ".quake-code", "skills");
	const globalDir = join(homedir(), ".quake-code", "skills");
