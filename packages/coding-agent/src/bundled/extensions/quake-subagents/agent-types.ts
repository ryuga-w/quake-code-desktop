 ...
 *
 * Merges embedded default agents with user-defined agents from .quake-code/agents/*.md.
...
import { type AgentConfig, CODEX_AGENT_NAMES } from "./types.js";
...
export function getCodexRoleNames(): string[] {
	const builtins = new Set<string>(CODEX_AGENT_NAMES);
	const custom = [...agents.entries()]
		.filter(([name, config]) => config.enabled !== false && (!config.isDefault || builtins.has(name)))
		.map(([name]) => name)
		.filter((name) => !builtins.has(name))
		.sort();
	const enabledBuiltins = CODEX_AGENT_NAMES.filter((name) => agents.get(name)?.enabled !== false);
	return [...custom, ...enabledBuiltins];
}

export function getCodexRoleDescription(): string {
	const roles = getCodexRoleNames().map((name) => {
		const config = agents.get(name);
		if (!config?.description) return `${name}: no description`;
		const lockedModel = config.model
			? `\n- This role's model is set to \`${config.model}\` and cannot be changed.`
			: "";
		const lockedReasoning = config.thinking
			? `\n- This role's reasoning effort is set to \`${config.thinking}\` and cannot be changed.`
			: "";
		return `${name}: {\n${config.description}${lockedModel}${lockedReasoning}\n}`;
	});
...
export function getMemoryTools(cwd: string, existingToolNames: Set<string>): AgentTool<any>[] {
	return MEMORY_TOOL_NAMES.filter((n) => !existingToolNames.has(n) && n in TOOL_FACTORIES).map((n) =>
		TOOL_FACTORIES[n](cwd),
	);
}
...
export function getReadOnlyMemoryTools(cwd: string, existingToolNames: Set<string>): AgentTool<any>[] {
	return READONLY_MEMORY_TOOL_NAMES.filter((n) => !existingToolNames.has(n) && n in TOOL_FACTORIES).map((n) =>
		TOOL_FACTORIES[n](cwd),
	);
}
