...
class TreeList implements Component {
	...
	selectMouseIndex(index: number): void {
		if (index < 0 || index >= this.filteredNodes.length) return;
		this.selectedIndex = index;
	...
	getStructureKey(): string {
		return `${this.selectedIndex}|${this.filteredNodes.length}|${this.searchQuery}|${this.filterMode}`;
	}
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			this.itemLineRegions.push({ index: i, lineStart, lineEnd: lineStart + 1 });
		}
	...
	private getEntryDisplayText(node: SessionTreeNode, isSelected: boolean): string {
		...
		switch (entry.type) {
			...
			case "compaction": {
				...
				result = theme.fg("accent", `✦ CONTEXT SUMMARIZED [ ${tokens}k tokens ]`);
				break;
			}
			case "branch_summary":
 ...
 */
export class TreeSelectorComponent extends Container implements Focusable, OverlayInteractiveTarget {
	...
	constructor(
		...
		const hint = `${theme.fg("dim", "↑/↓")} ${theme.fg("muted", "move · ")}${theme.fg("dim", "←/→")} ${theme.fg("muted", "page · ")}${theme.fg("dim", "Enter")} ${theme.fg("muted", "select · ")}${theme.fg("dim", "Esc")} ${theme.fg("muted", "close")}`;
		const footerHint = `^←/^→ fold/branch · ${keyText("app.tree.editLabel")} label · ^D/^T/^U/^L/^A filters · Esc · × close`;

		const frame = new SelectorFrame({
			title: "Session Tree",
			subtitle: "Navigate to any point in the session history",
			hint,
			footerHint,
			onClose: () => onCancel(),
		});
		...
		body.addChild(new SearchLine(this.treeList));
		body.addChild(new Spacer(1));
		body.addChild(this.treeContainer);
		body.addChild(this.labelInputContainer);
	...
	render(width: number): string[] {
		...
		if (!this.labelInput) {
			const listOnly = this.treeList.render(width);
	...
	collectOverlayContentRegions(width: number): HitRegion[] {
		...
		return this.overlayRegionCache.get(width, this.treeList.getStructureKey(), () => {
			if (width !== this.lastRenderWidth) this.render(width);
			return this.treeList.toItemRegions("tree", this.overlayListPrefix, width, this);
		});
	}

	setMouseHoverIndex(index: number | null): void {
		if (this.labelInput) return;
		this.treeList.setMouseHoverIndex(index);
	}

	selectMouseIndex(index: number): void {
		if (this.labelInput) return;
		this.treeList.selectMouseIndex(index);
	}

	scrollByWheel(direction: "up" | "down"): void {
		if (this.labelInput) return;
		this.treeList.scrollByWheel(direction);
	}
