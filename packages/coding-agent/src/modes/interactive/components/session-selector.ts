...
function isSameWorkspace(a: string | undefined, b: string | undefined): boolean {
...
class SessionSelectorHeader implements Component {
	...
	render(width: number): string[] {
		const title = this.loading
			? `Resume Session · ${this.loadProgress ? `${this.loadProgress.loaded}/${this.loadProgress.total}` : "loading"}`
			: `Resume Session · ${this.resultCount} session${this.resultCount === 1 ? "" : "s"}`;
		...
		if (this.confirmingDeletePath !== null) {
			...
			line2 = theme.fg(color, truncateToWidth(this.statusMessage.message, width, "…"));
			line3 = "";
		} else {
			line2 = theme.fg("dim", truncateToWidth("Type to filter sessions", width, "…"));
			const scopeText =
				this.scope === "current"
					? `${theme.fg("accent", "current folder")}`
					: `${theme.fg("accent", "all sessions")}`;
			const sortLabel = this.sortMode === "threaded" ? "threaded" : this.sortMode === "recent" ? "recent" : "fuzzy";
			const hints = [`scope ${scopeText}`, `sort ${theme.fg("muted", sortLabel)}`, `esc close`].filter(
				Boolean,
			) as string[];
			line3 = truncateToWidth(hints.join(theme.fg("dim", " · ")), width, "…");
		}

		return [leftText, line2, line3];
	}
}
...
class SessionList implements Component, Focusable {
	...
	private confirmingDeletePath: string | null = null;
	private pendingCrossWorkspacePath: string | null = null;
	...
	setSessions(sessions: SessionInfo[], showCwd: boolean): void {
		this.allSessions = sessions;
		this.showCwd = showCwd;
		this.pendingCrossWorkspacePath = null;
		this.filterSessions(this.searchInput.getValue());
	}
	...
	private requiresCrossWorkspaceConfirmation(session: SessionInfo): boolean {
		return this.showCwd && !!session.cwd && !!this.currentCwd && !isSameWorkspace(session.cwd, this.currentCwd);
	}
	...
	selectMouseIndex(index: number): void {
		if (index < 0 || index >= this.filteredSessions.length) return;
		this.selectedIndex = index;
		const selected = this.filteredSessions[index];
	...
	getStructureKey(): string {
		return `${this.selectedIndex}|${this.filteredSessions.length}|${this.searchInput.getValue()}|${this.sortMode}|${this.nameFilter}`;
	}
	...
	render(width: number): string[] {
		...
		// Render visible sessions in a compact Claude Code-like two-line list
		for (let i = startIndex; i < endIndex; i++) {
			...
			const fileSize = (() => {
				try {
					const size = existsSync(session.path) ? Math.round((statSync(session.path).size / 1024) * 10) / 10 : 0;
					return size > 0 ? `${size}KB` : "0KB";
				} catch {
					return "0KB";
				}
			})();
			const workspaceLabel = this.requiresCrossWorkspaceConfirmation(session)
				? theme.fg("warning", "other folder")
				: theme.fg("dim", "current folder");
			const cwdPart =
				this.showCwd && session.cwd ? `${shortenPath(session.cwd)} · ${workspaceLabel}` : workspaceLabel;
			const meta = `${cwdPart} · ${age} · HEAD · ${fileSize}${this.showPath ? ` · ${shortenPath(session.path)}` : ""}`;
			const cursor = isSelected || isHovered ? theme.fg("accent", "✦ ") : "  ";
			let titleColor: "error" | "warning" | null = null;
			if (isConfirmingDelete) titleColor = "error";
			else if (hasName) titleColor = "warning";
			const titleText = titleColor ? theme.fg(titleColor, displayText) : displayText;
			const line1Raw = cursor + (isSelected || isHovered ? theme.bold(titleText) : titleText);
			const line2Raw = `  ${theme.fg("muted", meta)}`;
			let line1 = truncateToWidth(line1Raw, width, "…");
			let line2 = truncateToWidth(line2Raw, width, "…");
			if (isSelected && this.pendingCrossWorkspacePath === session.path) {
				line2 = theme.fg(
					"warning",
					truncateToWidth(`  Other folder session. Press Enter again to resume · Esc to cancel`, width, "…"),
				);
			}
			if (isSelected) {
				line1 = theme.bg("selectedBg", line1.padEnd(Math.max(0, width), " "));
				line2 = theme.bg("selectedBg", line2.padEnd(Math.max(0, width), " "));
			} else if (isHovered) {
				line1 = applyMouseHoverStyle(line1);
				line2 = applyMouseHoverStyle(line2);
			}
			this.itemLineRegions.push({ index: i, lineStart: lines.length, lineEnd: lines.length + 3 });
			lines.push(line1);
			lines.push(line2);
			lines.push("");
		}
	...
	handleInput(keyData: string): void {
		...
		// Enter
			...
			if (selected && this.onSelect) {
				if (
					this.requiresCrossWorkspaceConfirmation(selected.session) &&
					this.pendingCrossWorkspacePath !== selected.session.path
				) {
					this.pendingCrossWorkspacePath = selected.session.path;
					return;
				}
		...
		// Escape - cancel
			...
			if (this.pendingCrossWorkspacePath !== null) {
				this.pendingCrossWorkspacePath = null;
				return;
			}
			if (this.onCancel) {
 ...
 */
export class SessionSelectorComponent extends Container implements Focusable, OverlayInteractiveTarget {
	...
	private buildBaseLayout(content: Component, options?: { showHeader?: boolean }): void {
		...
		const frame = new SelectorFrame({
			title: this.mode === "rename" ? "Rename Session" : "Resume Session",
			subtitle: this.mode === "rename" ? "Update the session display name" : "Browse and resume a previous session",
			hint:
				this.mode === "rename"
					? `${theme.fg("dim", keyText("tui.select.confirm"))} ${theme.fg("muted", "save · ")}${theme.fg("dim", keyText("tui.select.cancel"))} ${theme.fg("muted", "cancel")}`
					: `${theme.fg("dim", "↑↓")} ${theme.fg("muted", "navigate · ")}${theme.fg("dim", "Enter")} ${theme.fg("muted", "resume · ")}${theme.fg("dim", "Esc")} ${theme.fg("muted", "close")}`,
			footerHint:
				this.mode === "rename"
					? "Esc · × cancel"
					: "Type to search · Ctrl+R rename · Ctrl+D delete · Esc · × close",
			onClose: () => this.onCancel?.(),
		});
		const body = frame.getBody();
		this.addChild(frame);
		if (options?.showHeader ?? true) {
			body.addChild(this.header);
	...
	private async loadScope(scope: SessionScope, reason: "initial" | "refresh" | "toggle"): Promise<void> {
		...
		try {
			...
			this.header.setResultCount(sessions.length);
			this.sessionList.setSessions(sessions, showCwd);
			this.requestRender();
	...
	render(width: number): string[] {
		...
		if (this.mode === "list") {
			const listOnly = this.sessionList.render(width);
	...
	collectOverlayContentRegions(width: number): HitRegion[] {
		...
		return this.overlayRegionCache.get(width, this.sessionList.getStructureKey(), () => {
			if (width !== this.lastRenderWidth) this.render(width);
			return this.sessionList.toItemRegions("session", this.overlayListPrefix, width, this);
		});
	}

	setMouseHoverIndex(index: number | null): void {
		if (this.mode !== "list") return;
		this.sessionList.setMouseHoverIndex(index);
	}

	selectMouseIndex(index: number): void {
		if (this.mode !== "list") return;
		this.sessionList.selectMouseIndex(index);
	}

	scrollByWheel(direction: "up" | "down"): void {
		if (this.mode !== "list") return;
		this.sessionList.scrollByWheel(direction);
	}
}
