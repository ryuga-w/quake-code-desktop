...
import { applyMouseHoverStyle, type EditorTheme, MarkdownTheme, SelectListTheme } from "@mrquake/quakecode-tui";
...
const ThemeJsonSchema = Type.Object({
	...
	colors: Type.Object({
		...
		// Message rail (1 color)
		messageRail: ColorValueSchema,
	}),
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		for (const entry of entries) {
			...
			const themeName = entry.slice(0, -5);
			BUILTIN_THEMES[themeName] = JSON.parse(fs.readFileSync(themePath, "utf-8")) as ThemeJson;
		}
	}
...
const THEME_KEY = Symbol.for("@mrquake/quakecode-cli:theme");
...
export function getSelectListTheme(): SelectListTheme {
	return {
		...
		hoverText: (text: string) => applyMouseHoverStyle(theme.bold(theme.fg("accent", text))),
...
export function getEditorTheme(): EditorTheme {
	return {
		...
		placeholderColor: (text: string) => theme.fg("dim", text),
		selectList: getSelectListTheme(),
	};
}

export function getSettingsListTheme(): import("@mrquake/quakecode-tui").SettingsListTheme {
	return {
