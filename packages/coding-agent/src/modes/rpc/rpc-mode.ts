...
export async function runRpcMode(runtimeHost: AgentSessionRuntimeHost): Promise<never> {
	...
	const createExtensionUIContext = (): ExtensionUIContext => ({
		...
		requestUserInput: (request) =>
			createDialogPromise(
				...
				undefined,
				{ method: "requestUserInput", request },
				(r) => {
		...
		setSidebar(_key: string, _content: unknown): void {
			// Sidebar is an interactive TUI affordance and is not supported in RPC mode.
		},
	...
	const handleCommand = async (command: RpcCommand): Promise<RpcResponse> => {
		...
		switch (command.type) {
			...
			case "compact": {
				const result = await session.compact(command.customInstructions as any);
