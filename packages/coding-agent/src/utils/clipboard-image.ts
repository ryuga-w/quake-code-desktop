...
/**
 * On Windows (native or WSL), the Linux clipboard (Wayland/X11) does not receive image data from
 * Windows screenshots (Win+Shift+S). PowerShell can access the Windows clipboard
 * directly, so we use it as a fallback.
 */
function readClipboardImageViaPowerShell(isWsl: boolean): ClipboardImage | null {
	const tmpFile = join(tmpdir(), `quake-code-win-clip-${randomUUID()}.png`);
	let winPath = tmpFile;
	...
	try {
		...
		const psScript = [
			...
			"$path = $env:QUAKE_CODE_CLIPBOARD_IMAGE_PATH",
			"$img = [System.Windows.Forms.Clipboard]::GetImage()",
			"if ($img) { $img.Save($path, [System.Drawing.Imaging.ImageFormat]::Png); Write-Output 'ok' } else { Write-Output 'empty' }",
		].join("; ");

		const psCommand = isWsl ? "powershell.exe" : "powershell";
		const result = runCommand(psCommand, ["-NoProfile", "-Command", psScript], {
			timeoutMs: DEFAULT_POWERSHELL_TIMEOUT_MS,
			env: { ...process.env, QUAKE_CODE_CLIPBOARD_IMAGE_PATH: winPath },
		});
