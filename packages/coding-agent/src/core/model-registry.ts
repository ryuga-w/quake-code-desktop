...
import { registerOAuthProvider, resetOAuthProviders } from "@mrquake/quakecode-ai/oauth";
...
/** Free/local providers — always listed without auth.json. */
const ALWAYS_AVAILABLE_PROVIDERS = new Set<string>(["proxy-free", "opencode-free", "9router"]);

/** Grok family — available when env or ~/.grok auth is configured. */
const GROK_AUTH_PROVIDERS = new Set<string>(["grok-cli", "grok", "xai"]);
...
export class ModelRegistry {
		...
		for (const [providerName, providerConfig] of Object.entries(config.providers)) {
			...
			for (const modelDef of modelDefs) {
				...
				models.push({
					...
					headers: undefined,
					defaultQuery: providerConfig.defaultQuery,
					compat,
				} as Model<Api>);
			}
		}
	...
	hasConfiguredAuth(model: Model<Api>): boolean {
		if (ALWAYS_AVAILABLE_PROVIDERS.has(model.provider)) return true;
		if (GROK_AUTH_PROVIDERS.has(model.provider)) {
			return !!getEnvApiKey(model.provider as KnownProvider);
		}
		return (
			this.authStorage.hasAuth(model.provider) ||
			this.providerRequestConfigs.get(model.provider)?.apiKey !== undefined ||
			!!getEnvApiKey(model.provider as KnownProvider)
		);
	}
		...
		if (config.models && config.models.length > 0) {
			...
			this.models = this.models.map((m) => {
				...
				return {
					...
					defaultQuery: config.defaultQuery ?? m.defaultQuery,
				};
			});
		}
	}
