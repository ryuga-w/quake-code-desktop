...
function formatFindCall(
	...
	const title =
		path === null
			? `${theme.fg("muted", "Scanning files")} ${invalidArg}`
			: `${theme.fg("muted", "Scanning files under")} ${theme.fg("accent", path)}`;
	const subtitleParts = [
		pattern === null ? invalidArg : theme.fg("accent", pattern || "*"),
		limit !== undefined ? theme.fg("muted", `limit ${limit}`) : undefined,
	].filter(Boolean);
	return joinToolSections(title, formatToolMeta(theme, "find", status), subtitleParts.join(theme.fg("dim", " · ")));
}
...
function formatFindResult(
	...
	const sections: string[] = [
		status === "error"
			? theme.fg("error", "Find failed")
			: theme.fg(
					"muted",
					matchCount > 0 ? `Found ${matchCount} path${matchCount === 1 ? "" : "s"}` : "Completed with no matches",
				),
	];
	if (output) {
...
export function createFindToolDefinition(
	...
	return {
		...
		renderCall(args, theme, context) {
			...
			text.setText(formatFindCall(args, theme, status));
			return text;
		},
		renderResult(result, options, theme, context) {
			...
			text.setText(formatFindResult(result as any, options, theme, context.showImages, status));
			return text;
		},
	};
}
...
export const DEFAULT_FIND_TIMEOUT = 60;
