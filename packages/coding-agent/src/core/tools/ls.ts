...
function formatLsCall(
	...
	const title = `${theme.fg("muted", "Listing")} ${path === null ? invalidArg : theme.fg("accent", path)}`;
	const subtitle =
		limit !== undefined
			? theme.fg("muted", `Directory contents · limit ${limit}`)
			: theme.fg("muted", "Inspecting available files and folders");
	return joinToolSections(title, formatToolMeta(theme, "ls", status), subtitle);
}
...
function formatLsResult(
	...
	const sections: string[] = [
		status === "error"
			? theme.fg("error", "Directory listing failed")
			: theme.fg(
					"muted",
					entryCount > 0
						? `Listed ${entryCount} entr${entryCount === 1 ? "y" : "ies"}`
						: "Completed with no entries",
				),
	];
	if (output) {
...
export function createLsToolDefinition(
	...
	return {
		...
		renderCall(args, theme, context) {
			...
			text.setText(formatLsCall(args, theme, status));
			return text;
		},
		renderResult(result, options, theme, context) {
			...
			text.setText(formatLsResult(result as any, options, theme, context.showImages, status));
			return text;
		},
	};
}
