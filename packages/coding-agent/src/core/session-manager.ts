...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, undefined/false if Quake-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, false if Quake-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.quake-code/agent/sessions/.
 */
	...
	try {
		...
		for (const entry of entries) {
			...
			if (message.role === "user") {
				...
				lastModel = { provider: message.provider, modelId: message.model };
			}
		}
...
export class SessionManager {
	...
	_persist(entry: SessionEntry): void {
		...
		// Flush on first user or assistant message (not empty/new-only sessions).
		// Desktop history needs the chat listed as soon as the user writes;
		// waiting for assistant left failed/slow turns invisible in the sidebar.
		const hasUserOrAssistant = this.fileEntries.some(
			(e) => e.type === "message" && (e.message.role === "user" || e.message.role === "assistant"),
		);
		if (!hasUserOrAssistant) {
			// Mark as not flushed so when a real message arrives, all entries get written
			this.flushed = false;
			return;
		}
	...
	createBranchedSession(leafId: string): string | undefined {
		...
		if (this.persist) {
			...
			// Write when the branch already has a user/assistant message.
			// Otherwise defer to _persist() on the next real message so empty
			// forks don't create orphan files (and _persist can still do a
			// full first write without duplicating the header).
			const hasUserOrAssistant = this.fileEntries.some(
				(e) => e.type === "message" && (e.message.role === "user" || e.message.role === "assistant"),
			);
			if (hasUserOrAssistant) {
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.quake-code/agent/sessions/<encoded-cwd>/).
	 */
