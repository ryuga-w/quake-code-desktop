...
describe("AgentSessionRuntimeHost characterization", () => {
	...
	async function createRuntimeHost(
		...
		const bootstrap: AgentSessionRuntimeBootstrap = {
			...
			resourceLoader: {
				extensionFactories: [
					(pi) => {
						quake.registerProvider(faux.getModel().provider, {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((_pi) => {
			quake.on("session_before_switch", (event) => {
				events.push(event);
			});
			quake.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtimeHost } = await createRuntimeHost((_pi) => {
			quake.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
