...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const cachedDir = join(tmpdir(), "quake-code-extensions", `git-${gitHost}`, hash, gitPath);
			const extensionFile = join(cachedDir, "quake-code-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "quake-code-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ pi: { extensions: ["./quake-code-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "quake-code-extensions/session-breakdown.ts")).toBe("// fresh");
		});
		...
		it("should not refresh pinned temporary git sources", async () => {
			...
			expect(getFileContent(cachedDir, "quake-code-extensions/session-breakdown.ts")).toBe("// pinned");
		});
	});
