...
Project skill`,
	...
	quake.registerCommand("project-only", {
	...
	quake.registerCommand("user-only", {
		...
		handler: async () => {},
			...
			const discoveredTempExtensions = extensionsResult.extensions.filter((ext) => ext.path.startsWith(tempDir));
			expect(discoveredTempExtensions).toHaveLength(2);
...
export default function(pi: ExtensionAPI) {
  ...
  quake.registerCommand("deploy", {
    ...
    handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			const explicitIndex = extensionsResult.extensions.findIndex((ext) => ext.path === explicitExtPath);
