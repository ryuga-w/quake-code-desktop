...
describe("skills", () => {
	...
	describe("formatSkillsForPrompt", () => {
		...
		it("should include Codex-style bash loading instructions before XML", () => {
			const skills: Skill[] = [
				createTestSkill({
					name: "test-skill",
					description: "A test skill.",
			...
			expect(introText).toContain("Use bash with cat or sed to load a skill's file");
		});

		it("should describe read when bash is unavailable", () => {
			const skills: Skill[] = [
				createTestSkill({
					name: "test-skill",
					description: "A test skill.",
			...
			const result = formatSkillsForPrompt(skills, "read");
	...
	describe("loadSkills with options", () => {
		...
		it("should expand ~ in skillPaths", () => {
			...
			const { skills: withTilde } = loadSkills({
				agentDir: emptyAgentDir,
				cwd: emptyCwd,
				skillPaths: ["~/.quake-code/agent/skills"],
			});
