...
import { registerOAuthProvider } from "@mrquake/quakecode-ai/oauth";
...
describe("AuthStorage", () => {
	...
	describe("API key resolution", () => {
		...
		test("legacy OpenAI Codex ChatGPT auth is read as OAuth credentials", async () => {
			const expiresSeconds = Math.floor((Date.now() + 60_000) / 1000);
			writeAuthJson({
				"openai-codex": {
					type: "chatgpt",
			...
			expect(authStorage.get("openai-codex")).toMatchObject({ type: "oauth", expires: expiresSeconds * 1000 });
			await expect(authStorage.getApiKey("openai-codex")).resolves.toBe("legacy-access-token");
		});
