...
import { readTool, readToolDefinition } from "../src/core/tools/read.js";
...
describe("Coding Agent Tools", () => {
	...
	describe("Codex-style text inspection guidance", () => {
		it("directs routine text reads to bash while preserving read for images and paging", () => {
			const bashGuidance = bashToolDefinition.promptGuidelines?.join("\n") ?? "";
			const readGuidance = readToolDefinition.promptGuidelines?.join("\n") ?? "";

			expect(bashGuidance).toContain("cat, sed, nl, head, tail, rg, wc, and git show");
			expect(bashGuidance).toContain("separate parallel tool calls");
			expect(readGuidance).toContain("Codex-style text inspection");
			expect(readGuidance).toContain("Use read for images");
		});
	});
