...
describe("createBranchedSession", () => {
	...
	it("does not duplicate entries when forking from first user message", () => {
		...
		try {
			...
			// Fork from the very first user message (user present → file written immediately)
			const newFile = session.createBranchedSession(id1);
			expect(newFile).toBeDefined();

			// User message is enough to persist (desktop history / early list)
			...
			// File must still have exactly one header and no duplicate IDs
			expect(existsSync(newFile!)).toBe(true);
