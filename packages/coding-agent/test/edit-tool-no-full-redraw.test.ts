...
import { Container, type Terminal, Text, TUI } from "@mrquake/quakecode-tui";
...
describe("edit tool TUI rendering", () => {
	...
	it("keeps a settled large diff collapsed without triggering a full TUI redraw", async () => {
...
`,
		...
		expect(settledRender).toContain("done");

		component.setExpanded(true);
		const expandedRender = stripAnsi(component.render(80).join("\n"));
