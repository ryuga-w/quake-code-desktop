 ...
 * - ~/.quake-code/agent/presets.json (global)
...
export default function presetExtension(quake: ExtensionAPI) {
	let presets: PresetsConfig = {};
	let activePresetName: string | undefined;
	let activePreset: Preset | undefined;

	// Register --preset CLI flag
	quake.registerFlag("preset", {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (preset.thinkingLevel) {
			quake.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = quake.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				quake.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.quake-code/agent/presets.json or .pi/presets.json",
				"warning",
			);
			return;
		}
		...
		if (result === "(none)") {
			...
			quake.setActiveTools(["read", "bash", "edit", "write"]);
	...
	quake.registerShortcut(Key.ctrlShift("u"), {
	...
	quake.on("turn_start", async () => {
		if (activePresetName) {
			quake.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
