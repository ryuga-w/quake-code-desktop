...
cp permission-gate.ts ~/.quake-code/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `quake.sendUserMessage()` for sending user messages from extensions |
...
| `event-bus.ts` | Inter-extension communication via `quake.events` |
