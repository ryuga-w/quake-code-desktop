...
export default function (quake: ExtensionAPI) {
	...
	quake.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	quake.on("turn_end", async (_event, ctx) => {
	...
	quake.registerCommand("rpc-input", {
	...
	quake.registerCommand("rpc-editor", {
	...
	quake.registerCommand("rpc-prefill", {
