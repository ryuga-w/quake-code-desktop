...
import type { HitRegion, MouseCollectContext } from "../spatial-index.js";
...
/** Inline image placeholder from clipboard paste (paired with registerImageMarker). */
const IMAGE_MARKER_REGEX = /\[i(\d+)\]/g;
const IMAGE_MARKER_SINGLE = /^\[i(\d+)\]$/;
...
function isImageMarkerSegment(segment: string): boolean {
	return IMAGE_MARKER_SINGLE.test(segment);
}

const _VISIBLE_SPACE_MARKER = " ";

function renderVisibleWhitespace(text: string): string {
	// Replace standard space with Non-Breaking Space to force terminal rendering
	return text.replace(/ /g, "\u00A0");
}

function renderCursorGrapheme(grapheme: string): string {
	return grapheme;
}

/**
 * Merges graphemes within registered paste markers and image placeholders `[iN]`
 * into single atomic segments (cursor, backspace, wrap).
 */
function segmentWithPasteAndImageMarkers(
	text: string,
	pasteIds: Set<number>,
	imageIds: Set<number>,
	...
	if (pasteIds.size > 0 && text.includes("[paste #")) {
		for (const m of text.matchAll(PASTE_MARKER_REGEX)) {
			const id = Number.parseInt(m[1]!, 10);
			if (!pasteIds.has(id)) continue;
			markers.push({ start: m.index!, end: m.index! + m[0].length });
		}
	}
	if (imageIds.size > 0 && text.includes("[i")) {
	...
	for (const seg of baseSegments) {
		while (markerIdx < merged.length && merged[markerIdx]!.end <= seg.index) {
			markerIdx++;
		}

		const marker = markerIdx < merged.length ? merged[markerIdx]! : null;
...
export function wordWrapLine(line: string, maxWidth: number, preSegmented?: Intl.SegmentData[]): TextChunk[] {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		const isWs = !isPasteMarker(grapheme) && !isImageMarkerSegment(grapheme) && isWhitespaceChar(grapheme);
		...
		if (
			isWs &&
			next &&
			(isPasteMarker(next.segment) || isImageMarkerSegment(next.segment) || !isWhitespaceChar(next.segment))
		) {
...
export class Editor implements Component, Focusable {
	...
	/** Registered `[iN]` clipboard-image placeholders (atomic in the editor). */
	...
	constructor(tui: TUI, theme: EditorTheme, options: EditorOptions = {}) {
		...
		this.noBorders = options.noBorders ?? false;
	...
	/** Segment text with paste- and image-placeholder awareness. */
	private segment(text: string): Iterable<Intl.SegmentData> {
		return segmentWithPasteAndImageMarkers(text, this.validPasteIds(), this.imageMarkerIds);
	}

	/** Treat `[i(id)]` as one grapheme for navigation and backspace (see coding-agent clipboard paste). */
	registerImageMarker(id: number): void {
		this.imageMarkerIds.add(id);
		this.tui.requestRender();
	}

	unregisterImageMarker(id: number): void {
		this.imageMarkerIds.delete(id);
		this.tui.requestRender();
	}
	...
	render(width: number): string[] {
		...
		// The editor still draws its own visible block cursor; the terminal cursor is
		// only used for IME/native positioning so left/right movement stays obvious
		// even if the terminal cursor is hidden or blink-invisible.
		const emitCursorMarker = this.focused && !this.autocompleteState;

		for (let i = 0; i < visibleLines.length; i++) {
			const layoutLine = visibleLines[i]!;
			const rawText = layoutLine.text;
			// Always use renderVisibleWhitespace for stable rendering
			let displayText = renderVisibleWhitespace(rawText);
			let lineVisibleWidth = visibleWidth(rawText);
			let cursorInPadding = false;

			// Draw caret only while focused (welcome board / overlays should not show a text cursor).
			if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined && this.focused) {
				const beforeRaw = rawText.slice(0, layoutLine.cursorPos);
				const afterRaw = rawText.slice(layoutLine.cursorPos);
				const before = renderVisibleWhitespace(beforeRaw);

				// Hardware cursor marker (zero-width, emitted before fake cursor for IME positioning).
				const marker = emitCursorMarker ? CURSOR_MARKER : "";

				if (afterRaw.length > 0) {
					const afterGraphemes = [...this.segment(afterRaw)];
					const firstGrapheme = afterGraphemes[0]?.segment || "";
					const restAfterRaw = afterRaw.slice(firstGrapheme.length);
					const cursor = `\x1b[7m${renderCursorGrapheme(firstGrapheme)}\x1b[0m`;
					displayText = before + marker + cursor + renderVisibleWhitespace(restAfterRaw);
				} else {
					const cursor = "\x1b[7m\u00A0\x1b[0m";
					const placeholder =
						rawText.length === 0 && this.placeholder
							? (this.theme.placeholderColor ?? ((value: string) => value))(this.placeholder)
							: "";
					displayText = before + marker + cursor + placeholder;
					lineVisibleWidth = lineVisibleWidth + 1 + visibleWidth(this.placeholder || "");
					if (lineVisibleWidth > contentWidth && paddingX > 0) {
						cursorInPadding = true;
					}
				}
			} else if (rawText.length === 0 && this.placeholder && !this.focused) {
				displayText = (this.theme.placeholderColor ?? ((value: string) => value))(this.placeholder);
				lineVisibleWidth = visibleWidth(this.placeholder);
			}

			// Add prompt to the first line, otherwise pad
			const linePrompt = i === 0 ? this.prompt : " ".repeat(promptWidth);
		...
		if (this.autocompleteState && this.autocompleteList) {
			const autocompleteResult = this.autocompleteList.render(contentWidth + promptWidth);
			for (const line of autocompleteResult) {
				const lineWidth = visibleWidth(line);
				const linePadding = " ".repeat(Math.max(0, contentWidth + promptWidth - lineWidth));
	...
	handleInput(data: string): void {
		...
		if (this.autocompleteState && this.autocompleteList) {
			...
			if (kb.matches(data, "tui.input.tab")) {
				...
				if (selected) {
					this.applyAutocompleteItem(selected, { submitSlashCommand: false });
				}
				return;
			}
			...
			if (kb.matches(data, "tui.select.confirm")) {
				const selected = this.autocompleteList.getSelectedItem();
				if (selected) {
					const submitted = this.applyAutocompleteItem(selected, { submitSlashCommand: true });
					if (!submitted) return;
				}
			}
		}
	...
	public handleAutocompleteWheel(direction: "up" | "down"): void {
		if (!this.autocompleteState || !this.autocompleteList) return;
		this.autocompleteList.scrollByWheel(direction);
		this.tui.requestRender();
	}

	public setAutocompleteMouseHover(filteredIndex: number | null): void {
		if (!this.autocompleteList) return;
		this.autocompleteList.setMouseHoverIndex(filteredIndex);
		if (filteredIndex !== null) {
			this.autocompleteList.setSelectedIndex(filteredIndex);
		}
	}

	public selectAutocompleteFilteredIndex(filteredIndex: number): boolean {
		if (!this.autocompleteState || !this.autocompleteList) return false;
		this.autocompleteList.setSelectedIndex(filteredIndex);
		const selected = this.autocompleteList.getSelectedItem();
		if (!selected) return false;
		return this.applyAutocompleteItem(selected, { submitSlashCommand: true });
	}

	/** Mouse hit regions for the autocomplete dropdown (relative to the dropdown's first line). */
	public collectAutocompleteMouseRegions(width: number): HitRegion[] {
		if (!this.autocompleteState || !this.autocompleteList) return [];
		const promptWidth = visibleWidth(this.prompt);
		const maxPadding = Math.max(0, Math.floor((width - 1) / 2));
		const paddingX = Math.min(this.paddingX, maxPadding);
		const contentWidth = Math.max(1, width - paddingX * 2 - promptWidth);
		const listWidth = contentWidth + promptWidth;
		const lines = this.autocompleteList.render(listWidth);
		return this.autocompleteList.collectMouseRegions({
			startLine: 0,
			width,
	...
	private applyAutocompleteItem(selected: SelectItem, options: { submitSlashCommand: boolean }): boolean {
		...
		if (this.autocompletePrefix.startsWith("/") && options.submitSlashCommand) {
			this.cancelAutocomplete();
			if (this.onChange) this.onChange(this.getText());
			this.submitValue();
			return true;
	...
	/** Autocomplete dropdown lines as rendered below the editor content (empty when inactive). */
	public getAutocompleteLines(width: number): string[] {
		if (!this.autocompleteState || !this.autocompleteList) {
			return [];
		}
		const promptWidth = visibleWidth(this.prompt);
		...
		for (const line of autocompleteResult) {
			const lineWidth = visibleWidth(line);
			const linePadding = " ".repeat(Math.max(0, contentWidth + promptWidth - lineWidth));
			lines.push(`${leftPadding}${line}${linePadding}${rightPadding}`);
		}
		return lines;
	}
