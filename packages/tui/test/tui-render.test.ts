...
class LifecycleVirtualTerminal extends VirtualTerminal {
  ...
  override write(data: string): void {
    if (data.includes("\x1b[?1049h")) this.lifecycle.push("enter-alt");
    if (data.includes("\x1b[?1049l")) this.lifecycle.push("leave-alt");
    super.write(data);
  }
}
...
describe("TUI differential rendering", () => {
  it("owns an alternate-screen frame for stable application scrolling", async () => {
    const terminal = new LoggingVirtualTerminal(30, 6);
    const tui = new TUI(terminal);
    const component = new TestComponent();
    ...
    tui.start();
    await terminal.flush();
    const startWrites = terminal.getWrites();
    assert.ok(
      startWrites.includes("\x1b[?1049h"),
      "TUI should enter the alternate screen",
    );
    assert.ok(
      startWrites.includes("\x1b[?1007s") && startWrites.includes("\x1b[?1007l"),
      "TUI should save and disable wheel-to-arrow translation",
    );

    terminal.clearWrites();
    tui.stop();
    const stopWrites = terminal.getWrites();
    assert.ok(
      stopWrites.includes("\x1b[?1049l"),
      "TUI should restore the primary screen",
    );
    assert.ok(
      stopWrites.includes("\x1b[?1007r"),
      "TUI should restore the terminal's alternate-scroll preference",
    );
  });

  it("enables and disables terminal input modes inside the alternate screen", async () => {
    const terminal = new LifecycleVirtualTerminal(30, 6);
    const tui = new TUI(terminal);
    ...
    assert.deepStrictEqual(terminal.lifecycle, [
      "enter-alt",
      "start",
      "stop",
      "leave-alt",
  ...
  it("redraws wheel-scrolled viewports without clearing terminal scrollback", async () => {
    ...
    tui.setViewportScrollOffset(3);
    tui.requestRender();
    await terminal.flush();

    assert.ok(
      terminal.getWrites().includes("\x1b[1;1H"),
      "Scrolled viewport should update the fixed screen frame",
    );
    assert.ok(
      !terminal.getWrites().includes("\x1b[2J") &&
        !terminal.getWrites().includes("\x1b[3J"),
      "Wheel scrolling must not clear the screen or scrollback",
    );
    ...
    terminal.clearWrites();
    tui.setViewportScrollOffset(0);
    tui.requestRender();
    await terminal.flush();

    assert.ok(
      terminal.getWrites().includes("\x1b[1;1H"),
      "Returning to the bottom should update the fixed screen frame",
    );
    assert.ok(
      !terminal.getWrites().includes("\x1b[2J") &&
        !terminal.getWrites().includes("\x1b[3J"),
      "Returning to the bottom must not clear the screen or scrollback",
    );
    assert.deepStrictEqual(terminal.getViewport(), [
      "Line 7",
      "Line 8",
  ...
  it("keeps a scrolled viewport anchored while content streams below it", async () => {
    const terminal = new LoggingVirtualTerminal(20, 5);
    const tui = new TUI(terminal);
    const component = new TestComponent();
    tui.addChild(component);
    ...
    terminal.clearWrites();
    component.lines.push("Line 12");
    tui.requestRender();
    await terminal.flush();

    assert.strictEqual(
      tui.getViewportScrollOffset(),
      4,
      "New lines should increase the bottom-relative offset",
    );
    assert.deepStrictEqual(terminal.getViewport(), [
      "Line 4",
      "Line 5",
    ...
    assert.strictEqual(
      terminal.getWrites(),
      "",
      "An unchanged anchored viewport should not be redrawn",
    );
