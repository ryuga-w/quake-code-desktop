...
async function generateModels() {
	...
	// Default context window is based on observed server limits (400s above ~272k), not marketing numbers.
	// GPT-5.5 is explicitly listed by OpenAI as 400K in Codex, so it overrides this default below.
	const CODEX_BASE_URL = "https://chatgpt.com/backend-api";
	const CODEX_CONTEXT = 272000;
	const GPT_5_5_CODEX_CONTEXT = 400000;
	...
	const codexModels: Model<"openai-codex-responses">[] = [
		...
		},
		{
			...
			input: ["text", "image"],
			cost: { input: 2.5, output: 15, cacheRead: 0.25, cacheWrite: 0 },
			contextWindow: GPT_5_5_CODEX_CONTEXT,
			maxTokens: CODEX_MAX_TOKENS,
		},
		{
			id: "gpt-5.4-mini",
	...
	const antigravityModels: Model<"google-gemini-cli">[] = [
		...
		{
			// Backend ID differs from UI label: "gemini-3-flash-agent" == "Gemini 3.5 Flash (High)"
		...
		{
			// Backend ID differs from UI label: "gemini-3.5-flash-low" == "Gemini 3.5 Flash (Medium)"
		...
		{
			// Backend ID differs from UI label: "gemini-3.5-flash-extra-low" == "Gemini 3.5 Flash (Low)"
	...
	// OpenRouter exposes many vendors under one API; split NVIDIA-tagged models into a
	// dedicated provider key for model selection and auth mapping (same OpenRouter endpoint).
	for (const model of allModels) {
		if (model.provider === "openrouter" && model.id.startsWith("nvidia/")) {
