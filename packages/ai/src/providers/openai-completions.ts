...
import { ensureFreshGrokAuthToken, getGrokCliVersion } from "../grok-auth.js";
...
/**
 * Extract thinking tags from text (for claude-haiku-4.5-thinking-agentic via 9router)
 * Returns { thinking, cleanText }
 */
function extractThinkingTags(text: string): { thinking: string | null; cleanText: string } {
	const thinkingMatch = text.match(/<thinking>([\s\S]*?)<\/thinking>/g);
	if (!thinkingMatch) {
		return { thinking: null, cleanText: text };
	}
	const thinkingContent = thinkingMatch
		.map((m) => m.replace(/<\/?thinking>/g, ""))
		.join("\n\n")
		.trim();
	const cleanText = text.replace(/<thinking>[\s\S]*?<\/thinking>/g, "").trim();
	return { thinking: thinkingContent || null, cleanText };
}
...
export const streamOpenAICompletions: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		try {
			if (
				!options?.apiKey &&
				(model.provider === "grok-cli" || model.provider === "grok" || model.provider === "xai")
			) {
				await ensureFreshGrokAuthToken();
			}
			...
			const finishCurrentBlock = (block?: typeof currentBlock) => {
				if (block) {
					if (block.type === "text") {
						// Extract thinking tags from complete text (for claude-haiku-4.5-thinking-agentic)
						const { thinking, cleanText } = extractThinkingTags(block.text);
						if (thinking) {
							// Create thinking block and insert before text block
							const thinkingBlock: ThinkingContent = { type: "thinking", thinking };
							output.content.splice(blockIndex(), 0, thinkingBlock);
							block.text = cleanText;
							stream.push({ type: "thinking_start", contentIndex: blockIndex() - 1, partial: output });
							stream.push({
								type: "thinking_end",
								contentIndex: blockIndex() - 1,
								content: thinking,
								partial: output,
							});
						}
...
function createClient(
	...
	if (model.provider === "grok-cli" || model.baseUrl.includes("cli-chat-proxy.grok.com")) {
		Object.assign(headers, {
			"X-XAI-Token-Auth": "xai-grok-cli",
			"x-grok-model-override": model.id,
			"x-grok-client-version": getGrokCliVersion(),
		});
	}
...
function detectCompat(model: Model<"openai-completions">): Required<OpenAICompletionsCompat> {
	...
	const isGrokCli = provider === "grok-cli" || baseUrl.includes("cli-chat-proxy.grok.com");
	const isGrok = provider === "xai" || provider === "grok" || baseUrl.includes("api.x.ai");
