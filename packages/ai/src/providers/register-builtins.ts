...
import type { MiMoFreeOptions } from "./mimo-free.js";
...
interface MiMoFreeProviderModule {
	streamMiMoFree: StreamFunction<"mimo-free", MiMoFreeOptions>;
	streamSimpleMiMoFree: StreamFunction<"mimo-free", SimpleStreamOptions>;
}
...
let bedrockProviderModulePromise:
	...
	| undefined;
let mimoFreeProviderModulePromise:
	| Promise<LazyProviderModule<"mimo-free", MiMoFreeOptions, SimpleStreamOptions>>
	| undefined;
...
function loadMiMoFreeProviderModule(): Promise<LazyProviderModule<"mimo-free", MiMoFreeOptions, SimpleStreamOptions>> {
	mimoFreeProviderModulePromise ||= import("./mimo-free.js").then((module) => {
		const provider = module as MiMoFreeProviderModule;
		return {
			stream: provider.streamMiMoFree,
			streamSimple: provider.streamSimpleMiMoFree,
		};
	});
	return mimoFreeProviderModulePromise;
}
...
const streamMiMoFreeLazy = createLazyStream(loadMiMoFreeProviderModule);
const streamSimpleMiMoFreeLazy = createLazySimpleStream(loadMiMoFreeProviderModule);
...
export function registerBuiltInApiProviders(): void {
	...
	registerApiProvider({
		api: "mimo-free",
		stream: streamMiMoFreeLazy,
		streamSimple: streamSimpleMiMoFreeLazy,
	});
}
