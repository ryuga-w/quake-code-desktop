...
function getGeminiCliOAuthClientCredentials(): { clientId: string; clientSecret: string } {
	const clientId = process.env.GOOGLE_GEMINI_CLI_OAUTH_CLIENT_ID;
	const clientSecret = process.env.GOOGLE_GEMINI_CLI_OAUTH_CLIENT_SECRET;
	if (!clientId || !clientSecret) {
		throw new Error("Gemini CLI OAuth requires GOOGLE_GEMINI_CLI_OAUTH_CLIENT_ID and GOOGLE_GEMINI_CLI_OAUTH_CLIENT_SECRET.");
	}
	...
	return new Promise((resolve) => {
		...
		server
			...
			.on("error", (err: NodeJS.ErrnoException) => {
				console.error(
					"[google-gemini-cli] Failed to bind http://127.0.0.1:8085 (",
					err.code,
...
export async function refreshGoogleCloudToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	const response = await fetch(TOKEN_URL, {
		...
		body: new URLSearchParams({
			client_id: getGeminiCliOAuthClientCredentials().clientId,
			client_secret: getGeminiCliOAuthClientCredentials().clientSecret,
...
export async function loginGeminiCli(
	...
	try {
		...
		onAuth({
			url: authUrl,
			instructions:
				"Complete the sign-in in your browser. If localhost callback fails, copy the final redirect URL from the browser and paste it here.",
		});
