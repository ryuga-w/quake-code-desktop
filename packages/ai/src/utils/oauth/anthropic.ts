	...
	return new Promise((resolve) => {
		...
		server
			...
			.on("error", (err: NodeJS.ErrnoException) => {
				console.error(
					`[anthropic] Failed to bind http://127.0.0.1:${CALLBACK_PORT}${CALLBACK_PATH} (`,
					err.code,
...
export async function loginAnthropic(options: {
	...
	try {
		...
		options.onAuth({
			...
			instructions:
				"Complete login in your browser. If localhost callback fails or the browser is on another machine, paste the final redirect URL here.",
		});
