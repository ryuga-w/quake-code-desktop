...
async function createAuthorizationFlow(originator = "quake-code") {
...
function startLocalOAuthServer(state) {
    ...
    const server = _http.createServer((req, res) => {
        try {
            ...
            res.statusCode = 200;
            res.setHeader("Content-Type", "text/html; charset=utf-8");
            res.end(oauthSuccessHtml("Quake Code authentication completed. You can close this window."));
            settleWait?.({ code });
        }
        catch {
            res.statusCode = 500;
 ...
 * @param options.originator - OAuth originator parameter (defaults to "quake-code")
 */
