...
function getAntigravityOAuthClientCredentials(): { clientId: string; clientSecret: string } {
	...
	if (!clientId || !clientSecret) {
		throw new Error("Antigravity OAuth requires GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_ID and GOOGLE_ANTIGRAVITY_OAUTH_CLIENT_SECRET.");
	}
	...
	return new Promise((resolve) => {
		...
		server
			...
			.on("error", (err: NodeJS.ErrnoException) => {
				console.error(
					"[google-antigravity] Failed to bind http://127.0.0.1:51121 (",
					err.code,
...
export async function refreshAntigravityToken(refreshToken: string, projectId: string): Promise<OAuthCredentials> {
	const response = await fetch(TOKEN_URL, {
		...
		body: new URLSearchParams({
			client_id: getAntigravityOAuthClientCredentials().clientId,
			client_secret: getAntigravityOAuthClientCredentials().clientSecret,
