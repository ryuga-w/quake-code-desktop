import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel } from "@mrquake/quakecode-ai";
...
describe("Agent", () => {
	...
	it("abort clears steer and follow-up queues (Stop must not restart)", () => {
		const agent = new Agent();
		agent.steer({ role: "user", content: "steer me", timestamp: Date.now() });
		agent.followUp({ role: "user", content: "follow later", timestamp: Date.now() });
