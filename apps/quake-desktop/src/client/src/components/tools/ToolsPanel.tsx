...
export function ToolsPanel({ onOpenDiff, onInspect, onOpenFile }: { onOpenDiff: (card: ToolCardState) => void; onInspect: (card: ToolCardState) => void; onOpenFile?: (path: string) => void }) {
  ...
  const groups = useMemo(() => groupToolsByTurn(visibleTools), [visibleTools]);
  const nextToolCount = Math.min(hiddenToolCount, TOOL_PANEL_WINDOW_STEP);
  return <div className="panel"><div className="panel-title-row"><div className="panel-title">Araç akışı</div><span>{view.filtered}/{view.total}</span></div><input className="panel-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Araç, durum veya tur filtrele…" /><div className="segmented">{["all", "running", "streaming", "done", "error"].map((item) => <button key={item} type="button" className={status === item ? "active" : ""} onClick={() => setStatus(item)}>{statusFilterLabel(item)}</button>)}</div><div id="tools" className={`tool-list ${view.filtered ? "" : "muted"}`}>{view.filtered ? <>{groups.map((group) => <ToolTurnGroup group={group} key={group.turnId} onOpenDiff={onOpenDiff} onInspect={onInspect} onOpenFile={onOpenFile} />)}{hiddenToolCount > 0 && <button type="button" className="tree-load-more" onClick={() => setToolWindowSize((value) => value + TOOL_PANEL_WINDOW_STEP)}>Daha eski {nextToolCount} aracı göster <span>{hiddenToolCount} gizli</span></button>}</> : "Henüz araç çağrısı yok"}</div></div>;
}
...
export function ToolTurnGroup({ group, onOpenDiff, onInspect, onOpenFile }: { group: ToolTurnSummary; onOpenDiff: (card: ToolCardState) => void; onInspect: (card: ToolCardState) => void; onOpenFile?: (path: string) => void }) {
  return <section className="tool-turn-group"><div className="tool-turn-head"><div><strong>Tur #{group.turnId}</strong><span>{group.summary}</span></div><em>{group.durationMs !== undefined ? formatDuration(group.durationMs) : "çalışıyor"}</em></div>{group.tools.map((card) => <ToolCard card={card} key={card.id} onOpenDiff={onOpenDiff} onInspect={onInspect} onOpenFile={onOpenFile} />)}</section>;
}

export type ToolTurnSummary = { turnId: number | string; tools: ToolCardState[]; summary: string; durationMs?: number };

export function groupToolsByTurn(tools: ToolCardState[]): ToolTurnSummary[] {
  ...
  for (const [turnId, turnTools] of map) summaries.push(createToolTurnSummary(turnId, turnTools));
  return summaries;
}

export function createToolTurnSummary(turnId: number | string, turnTools: ToolCardState[]): ToolTurnSummary {
  let errors = 0;
  let running = 0;
  let completed = 0;
  ...
  for (const tool of turnTools) {
    ...
    const displayName = toolDisplayName(tool.toolName);
  ...
  const summary = `${turnTools.length} araç · ${completed} tamamlandı${running ? ` · ${running} çalışıyor` : ""}${errors ? ` · ${errors} hata` : ""}${nameSummary ? ` · ${nameSummary}` : ""}`;
...
export function ToolCard({ card, onOpenDiff, onInspect, onOpenFile }: { card: ToolCardState; onOpenDiff: (card: ToolCardState) => void; onInspect: (card: ToolCardState) => void; onOpenFile?: (path: string) => void }) {
  ...
  return <div className={`tool-card ${card.status}`}><details open={open} onToggle={(event) => setOpen(event.currentTarget.open)}><summary><span>{toolDisplayName(card.toolName)}</span><small>{summarizeToolArgs(card.toolName, card.args) || statusLabel(card.status)}</small></summary><div className="tool-actions"><button className="tool-action" type="button" onClick={() => onInspect(card)}>Detayı incele</button>{diff && <button className="tool-action" type="button" onClick={() => onOpenDiff(card)}>Diff aç</button>}</div><ToolRendererInline card={card} onOpenFile={onOpenFile} /></details></div>;
}
