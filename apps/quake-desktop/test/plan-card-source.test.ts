...
describe("Codex-compatible plan contracts", () => {
  ...
  it("renders Proposed Plan and Updated Plan without execution controls", () => {
    ...
    expect(artifactPanel).toContain('t("runtime.plan.updatedPlan")');
    expect(i18n).toContain('proposedPlan: "Önerilen plan"');
    expect(i18n).toContain('updatedPlan: "Güncellenen plan"');
  ...
  it("offers Codex-style apply or revise actions when a plan is ready", () => {
    ...
    expect(i18n).toContain('applyQuestion: "Bu plan uygulansın mı?"');
  });
  ...
  it("renders request_user_input through the Questions card", () => {
    ...
    expect(i18n).toContain('other: "Diğer…"');
